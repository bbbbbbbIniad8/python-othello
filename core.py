...
class othello:
    ...
    def draw_board_line(self):
        for YC in range(8):
            for XC in range(8):
                put_on_mass(self.screen, self.mass_size, self.shift, XC, YC, (0, 0, 0), "square")

    def draw_now_board(self, board_lst, search_lst):
        for YC in range(8):
            for XC in range(8):
                if board_lst[YC][XC] == 0:
                    put_on_mass(self.screen, self.mass_size, self.shift, XC, YC, self.RGBdict["BLACK"], "circle")
                elif board_lst[YC][XC] == 1:
                    put_on_mass(self.screen, self.mass_size, self.shift, XC, YC, self.RGBdict["WHITE"], "circle")
                elif board_lst[YC][XC] == -1 and search_lst[YC][XC] > 0:
                    put_on_mass(self.screen, self.mass_size, self.shift, XC, YC, (100, 100, 0), "small_square")

    def put_stone(self, board_lst, color, X, Y):
        putcolor = self.RGBdict["BLACK"] if color == 0 else self.RGBdict["WHITE"]
        if board_lst[Y][X] == -1:
            board_lst[Y][X] = color
            put_on_mass(self.screen, self.mass_size, self.shift, X, Y, putcolor, "circle")
            time.sleep(0.1)
            pass_ = 0
        return board_lst
    ...
    def check_change_stone(self, board_lst, color, mode, X, Y):
        ...
        for p in pies:
            ...
            if all([check_X not in ng_lst, check_Y not in ng_lst]) and board_lst[check_Y][check_X] == opposite_color:
                while True:
                    check_X += p[0]
                    check_Y += p[1]
    ...
    def AI_put_where(self, board_lst, evaluation_lst, color):
        ...
        best_score = -math.inf
        init_coord = (-1, -1)
        decide_put = [init_coord, best_score]
        for Y in range(8):
            for X in range(8):
                if (board_lst[Y][X] == -1 and search_lst[Y][X] > 0 and 
                    self.check_change_stone(board_lst, color, "cnt", X, Y) > 0 and decide_put[1] < evaluation_lst[Y][X]):
                        decide_put[0] = (X, Y)
                        decide_put[1] = evaluation_lst[Y][X]
        return decide_put[0]

    def count_stone(self, board_lst):
        black = 0
        white = 0
        none = 0
        for Y in range(8):
        ...
        while True:
            ...
            if turn % 2 == 0:
                ...
                if (event.type == pygame.MOUSEBUTTONDOWN and
                    ...
                    board_lst[Y][X] == -1 and self.check_change_stone(board_lst, now_put_color, "cnt", X, Y) >= 1):
                        board_lst = self.put_stone(board_lst, now_put_color, X, Y)
                        board_lst = self.check_change_stone(board_lst, now_put_color, "exe", X, Y)
                        turn += 1

                black, white, none = self.count_stone(board_lst)
